...
public class ChartStyle
{
    ...
    public ChartStyle(Form1 fm1)
    {
        ...
        chartBackColor = form1.BackColor;
    ...
    public string YTickDisplayFormat { get; set; } = "{0}";
    ...
    public void AddChartStyle(Graphics g)
    {
        ...
        if (IsYGrid == true)
        {
            ...
            for (fX = XLimMin + XTick; fX < XLimMax-0.001; fX += XTick)
            {
        ...
        if (IsXGrid == true)
        {
            aPen = new Pen(GridColor, 1f);
            aPen.DashStyle = GridPattern;
            for (fY = YLimMin + YTick; fY < YLimMax-0.001; fY += YTick)
            {
        ...
        for (fY = YLimMin; fY <= YLimMax; fY += YTick)
        {
            ...
            StringFormat sFormat = new StringFormat();
            sFormat.Alignment = StringAlignment.Far;
            g.DrawString(string.Format(YTickDisplayFormat, fY), TickFont, aBrush,  new PointF(xAxisPoint.X - 3f,
    ...
    private void AddLabels(Graphics g)
    {
        ...
        g.TranslateTransform(xOffset,  yOffset + titleFontSize.Height + yOffset / 3 + PlotArea.Height / 2);
    ...
    public PointF Point2D(PointF pt)
    {
        ...
        aPoint.X = PlotArea.X + (pt.X - XLimMin) *  PlotArea.Width / (XLimMax - XLimMin);
        aPoint.Y = PlotArea.Bottom - (pt.Y - YLimMin) * PlotArea.Height / (YLimMax - YLimMin);
        return aPoint;
    }
}
